/* nvd3 version 1.8.2-dev (https://github.com/novus/nvd3) 2016-03-25 */
(function(){
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            function updateLineLabels() {
                if(showLabelOnRight) {
                    var scaleMax = availableHeight1;
                    var bottomY = scaleMax + 5;

                    var checkLabelYScaleFunc = function(d) { return d.y; }
                    var maxLabelYScaleMax = data.reduce(function(accumulator, current) {
                        if(accumulator === null) {
                            return d3.max(current.values, checkLabelYScaleFunc)
                        };
                        return Math.max(d3.max(current.values, checkLabelYScaleFunc), accumulator);
                    }, null);

                    var minLabelYScaleMax = data.reduce(function(accumulator, current) {
                        if(accumulator === null) {
                            return d3.min(current.values, checkLabelYScaleFunc)
                        };
                        return Math.min(d3.min(current.values, checkLabelYScaleFunc), accumulator);
                    }, null);

                    var labelYScale = d3.scale.linear()
                        .domain([minLabelYScaleMax, maxLabelYScaleMax])
                        .range([0, scaleMax]);

                    g.select('.nv-linesWrap').selectAll('text').remove();

                    g.select('.nv-linesWrap').selectAll('text')
                        .data(data)
                        .enter()
                        .append('text')
                        .attr('x', function(d, i) {
                        ...
                        .attr('y', function(d) {
                          if(minLabelYScaleMax === maxLabelYScaleMax) {
                            return  bottomY - availableHeight1 / 2;
                          }
                          return bottomY - labelYScale(d.values[d.values.length - 1].y);
                        })
                        .style('font-size', 9)
                        .attr('fill', 'black')
                        .text(function(d) {
                            return d.shortLabel || d.key
                        })
                }
            }
